...
postgresExplainForeignScan(ForeignScanState *node, ExplainState *es)
{
	...
	if (es->verbose)
	{
		...
		/*
		 * Fetch and display remote plan if show_remote_plans is enabled.
		 *
		 * TODU: Support foreign joins (scanrelid == 0) in Phase 2.
		 * For now, skip joins because extracting serverid from fs_base_relids
		 * and verifying all tables are on same server adds complexity.
		 * Cross-server joins already work since they become local joins with
		 * separate Foreign Scans (each with scanrelid > 0).
		 */
		if (show_remote_plans && plan->scan.scanrelid > 0)
		{
			PgFdwScanState *fsstate = (PgFdwScanState *) node->fdw_state;
			PGconn	   *conn;
			PGresult   *res;
			...
			/*
			 * Get connection. For EXPLAIN without execution (fsstate is NULL),
			 * need to establish connection ourselves.
			 */
			...
			{
				/* EXPLAIN-only case: establish connection from user mapping */
				RangeTblEntry *rte;
				ForeignTable *table;
				ForeignServer *server;
				UserMapping *user;
				...
				/* Use checkAsUser if set, otherwise current user */
				userid = OidIsValid(plan->checkAsUser) ? plan->checkAsUser : GetUserId();
				rte = exec_rt_fetch(plan->scan.scanrelid, node->ss.ps.state);
				table = GetForeignTable(rte->relid);
				server = GetForeignServer(table->serverid);
			...
			/*
			 * Execute EXPLAIN on remote server to get the plan.
			 * Using text format (default) is simpler than JSON parsing and gives
			 * us complete plan info including nested nodes, costs, etc.
			 *
			 * TODU: For EXPLAIN ANALYZE support (Phase 2), need to:
			 * - Add ANALYZE option when instrumenting is enabled
			 * - Parse output to extract actual rows/time
			 * - Populate local instrumentation to avoid double execution
			 */
			initStringInfo(&explain_sql);
			appendStringInfo(&explain_sql, "EXPLAIN (VERBOSE, COSTS) %s", sql);

			/* Execute with error handling - don't let remote errors break EXPLAIN */
			PG_TRY();
			{
				...
				if (PQresultStatus(res) == PGRES_TUPLES_OK)
				{
					...
					if (nrows > 0)
					{
						...
						/* Display each line of the remote plan with indentation */
						for (int i = 0; i < nrows; i++)
						{
							remote_plan = PQgetvalue(res, i, 0);
							ExplainIndentText(es);
			...
			PG_CATCH();
			{
				...
				/* Capture error without propagating */
				edata = CopyErrorData();
				FlushErrorState();

				/* Warn but don't fail - remote server might be down */
				ereport(WARNING,
						(errmsg("could not fetch remote plan from foreign server"),
